...
class SpecialRequestWiki extends SpecialPage {
	...
	function addRequestWikiForm() {
		...
		$form .= '<td>' .
			Xml::languageSelector( 'en', true, null, array( 'name' => 'language' ) )[1] .
			'</td></tr>';
		...
		$form .= '<td>' .
			Xml::textarea( 'comments', '', 40, 5, array( 'required' => '' ) ) .
			'</td></tr>';
		$form .= '<tr><td>' .
			Xml::submitButton( $this->msg( 'requestwiki-submit' )->plain() ) .
			'</td></tr>';
	...
	function handleRequestWikiFormInput() {
		...
		if ( $this->errors ) {
			$out->addHTML(
				'<div class="errorbox">' .
				$this->msg( 'requestwiki-error-notallfilledin' )->escaped() .
				'</div>'
			);
		...
		if ( $subdomain && $customdomain ) {
			$out->addHTML(
				'<div class="errorbox">' .
				$this->msg( 'requestwiki-error-twodomains' )->escaped() .
				'</div>'
			);
			...
			$out->addHTML(
				'<div class="errorbox">' .
				$this->msg( 'requestwiki-error-nodomain' )->escaped() .
				'</div>'
			);
		...
		$idlink =
			Linker::link(
				Title::newFromText( 'Special:RequestWikiQueue/' . $dbw->insertId() ),
				"#{$dbw->insertId()}"
			);
		...
		$this->getOutput()->addHTML(
			...
			$this->msg( 'requestwiki-success', $idlink )->plain() .
			'</div>'
		);
	}
